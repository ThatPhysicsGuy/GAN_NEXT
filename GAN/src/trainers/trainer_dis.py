...
class GAN_trainer_dis():
    ...
    def __init__(self,batch, fn_dis, fn_sim, parameters, subkey):
        ...
        parameters_dis =  {x:parameters[x] for x in parameters.keys() if x == "D_network_params"}
        
        self.parameters_sim = {x:parameters[x] for x in parameters.keys() if x != "D_network_params"}
        
        print('Created Trainer',flush = True)
        
        def forward_pass_dis(batch, parameters_dis, key):

            simulated_pmts, simulated_sipms = self.sim_wf(batch['energy_deposits'], self.parameters_sim, self.noise, key)
            
            GAN_batch = self.Chanteclair(batch['S2Si'],simulated_sipms)
           
           
            fake_labels = self.dis_apply(parameters_dis["D_network_params"], GAN_batch['Train'])
        ...
        self.gradient_dis = jax.value_and_grad(forward_pass_dis, argnums=1,has_aux=False)
        
        self.opt_init, opt_update, get_params = jax_opt.adamax(1e-2)
            
        self.opt_state = self.opt_init(parameters_dis)
    ...
    def train_iteration(self, batch,c):
        ...
        parameters_dis = self.get_params(self.opt_state)
        
        self.key = random.PRNGKey(int(time.time()))

        self.key, subkey = jax.random.split(self.key)
        
        loss_dis, gradients_dis = self.gradient_dis(batch,parameters_dis, subkey)
    ...
    def parameters(self):
        ...
        p_dis = self.get_params(self.opt_state)
        # Deliberately slice things up here:
        
        parameters = {}
        parameters["D_network_params"] = p_dis["D_network_params"]
